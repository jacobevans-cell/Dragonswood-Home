...
print('Student World Python dependencies: READY')
PY
...
"${FIREBASE[@]}" emulators:exec --project demo-dragonswood-v33 --config firebase.integration.json --only auth,firestore "bash tools/run-student-world-emulator-tests.sh"
./tools/check-student-world.sh
python -u tools/visual-pixel-regression.py --kind student
python -u tools/visual-pixel-regression.py --kind teacher
echo 'V3.3 STUDENT WORLD WAVE: PASS (Firebase + My Day + Hall/pets + boss + leaderboards + 0-pixel visuals)'
