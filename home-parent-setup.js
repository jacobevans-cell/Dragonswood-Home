...
function renderForms(){
  formsRoot.innerHTML=slots.map(slot=>{
    const existing=profileById.get(slot.id)||{},avatarId=existing.avatarId||slot.avatarId;
    return `<form class="panel home-profile-form" data-profile-form="${slot.id}">
      ...
      <label>Preferred name or nickname<input name="nickname" maxlength="24" autocomplete="off" required value=""></label>
      <label>Profile picture<select name="avatarId">${options(avatars,avatarId)}</select></label>
      <label>School grade<select name="grade">${gradeOptions(slot.grade)}</select></label>
...
async function saveProfile(event,slot){
  ...
  try{
    const call=sdk.functionsModule.httpsCallable(functions,'configureHomeVisualProfile'),result=await call({profileId:slot.id,nickname:data.get('nickname'),avatarId:data.get('avatarId'),grade:Number(data.get('grade')),colorId:data.get('colorId'),animalId:data.get('animalId'),sortOrder:slot.sortOrder});
...
renderForms();
if(!configured){signInButton.disabled=true;setStatus('Connect the separate Dragonswood Home Firebase project first. This page is ready but cannot save yet.','warning')}
