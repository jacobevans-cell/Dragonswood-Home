import {environment,getArcadeAccess,startArcadeSession,setCurrentAccess,remainingMs} from './access-client.js?v=60.1.0';
let loaded=false;
...
function renderLocked(message='Checking the Arcade hourglass…'){
  ...
  const homeAccess=access?.homeAccess===true;
  const ready=homeAccess||afternoonAccess||testerOverride||(count===3&&enabled);
  document.documentElement.classList.remove('arcade-auth-pending');
  gate.hidden=false;
  badge.hidden=true;
  gate.innerHTML=`<div class="arcade-access-card"><img src="../v33-integration/assets/branding/dragonswood-mascot-crest.png" alt=""><h1>Arcade Time</h1>${homeAccess?'':tokens(count)}<p>${esc(message)}</p><p class="arcade-access-note">${homeAccess?'🏡 Home Arcade is always open. No Tokens or teacher unlocks are needed.':arcadeForAll?'🕹️ Free Arcade for Everyone is active. No work requirements and no Tokens.':afternoonAccess?'🎮 Afternoon Substitute free play is active. No Arcade Tokens will be charged.':testerOverride?'🧪 TRUE TESTER self-unlock is active. Your normal Token wallet will not be charged.':'3 Tokens = one 30-minute session. Tokens are earned for Ready, Responsible, and Complete choices. Wallet maximum: 3.'}</p><div class="arcade-access-actions"><button type="button" data-start-arcade ${ready?'':'disabled'}>${homeAccess?'Enter Home Arcade':arcadeForAll?'Start Free Arcade':afternoonAccess?'Start Free Afternoon Arcade':testerOverride?'Start Tester Session':'Use 3 Tokens — Start 30 Minutes'}</button><a href="${portalHref()}">Return to Dragonswood</a></div><p class="arcade-access-note">${homeAccess?'Open family play':arcadeForAll?'Whole-class free access':afternoonAccess?'Finished-work access':testerOverride?'Tester-only personal access':enabled?'Teacher Arcade Time is open.':'Teacher Arcade Time is currently locked.'}${environment==='production'?'':` • ${esc(environment)}`}</p></div>`;
...
async function begin(){
  ...
  if(btn){btn.disabled=true;btn.textContent=access?.homeAccess?'Opening Home Arcade…':access?.afternoonSubstituteAccess?'Starting free play…':'Starting Arcade Time…'}
